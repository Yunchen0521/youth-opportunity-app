...
流程：讀 sources.json → 逐一抓網頁 → 用 Claude 解析成 Opportunity 結構 →
...
# 交給 Claude 的目標結構（對齊 App 的 Opportunity；id/sourceType/location 由程式補）。
SCHEMA = {
    "type": "object",
    ...
    "properties": {
        "opportunities": {
            ...
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["title", "category", "organizer", "summary",
                             "description", "amount", "deadline", "applyStartDate",
                             "website", "eligibility"],
                "properties": {
                    "title": {"type": "string"},
...
def extract(client: Anthropic, text: str, source: dict) -> list[dict]:
    user = (f"來源：{source['name']}（sourceType={source['sourceType']}、"
            f"預設主辦：{source['organizer']}）\n\n網頁文字：\n{text[:20000]}")
    resp = client.messages.create(
        # 預設 opus-4-8；批次擷取想省成本可改 "claude-haiku-4-5"
...
def main() -> None:
    ...
    for src in sources:
        ...
        try:
            items = extract(client, fetch_text(src["url"]), src)
